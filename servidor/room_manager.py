...
user = str
room = NamedTuple('room', [('users',    set[user]),
                           ('messages', list[dict])])
...
def get_all_users(): #! deve ser devagar
    return set.union(*(r.users for r in rooms.values()))
...
def add_message(room_name, username, content, recipient=None):
    if (room_name not in rooms) or \
       (username  not in rooms[room_name].users): return None #! lidar com isso direito
